...
- 为 `TypeChecker` 增加最小 GTest 集（与 `diagnostic/codes.hpp` 语义错误码断言配合更佳）。
...
`codes.hpp` 中 `semantic` 命名空间当前主要为泛化错误码；语义细分错误若长期共用同一 `code`，不利于机器侧分类统计。属 **P1 体验/生态** 项，可在 P0 收口后再扩展细分码表。
