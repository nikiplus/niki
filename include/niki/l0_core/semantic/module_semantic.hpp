...
struct ImportBinding {
    ...
    uint32_t import_line = 0;  ///< import 声明行号（用于可见性解析诊断）。
    uint32_t import_column = 0;
};
...
struct ModuleExportTable {
    std::unordered_map<uint32_t, std::unordered_map<uint32_t, SymbolRef>>
        table; ///< module_id -> (export_name -> symbol)。
};
