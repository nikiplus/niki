...
void Compiler::compileDeclaration(ASTNodeIndex nodeIdx) {
    ...
    case NodeType::ErrorNode:
        ...
        break;
    default:
        // 顶层仅允许声明节点；未知节点一律报错，避免退回“语句兜底”。
        reportError(line, column, "Top-level only supports declarations.");
        break;
    }
}
...
void Compiler::compileStructDecl(ASTNodeIndex nodeIdx) {
    ...
    if (constIdx < 255) {
        ...
        emitOp(vm::OPCODE::OP_DEFINE_GLOBAL_W, static_cast<uint8_t>((constIdx >> 8) & 0xFF),
               static_cast<uint8_t>(constIdx & 0xFF), line, column);
    }
}
